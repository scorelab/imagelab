...
public class ScharrOperator extends OpenCVOperator {
	...
	public Mat compute(Mat image) {
		...
		System.out.println("Applied Scharr Derivation");
        return scharrDerivation(image,getDepth(),getType());
	}

	private Mat scharrDerivation(Mat src,int depth,int type) {
		  // Creating an empty matrix to store the result
	      Mat dst = new Mat();
	      switch(type) {
	      	case 1 :  Imgproc.Scharr(src, dst, depth, 0, 1);break;// capture edges with horizontal direction only
	      	case 2 :  Imgproc.Scharr(src, dst, depth, 1, 0);break;// capture edges with vertical direction only
	      	//case 3 :  Imgproc.Scharr(src, dst, depth, 1, 1);break;// capture both vertical and horizontal  
	      }
	...
	public enum Information {
        ...
        OPERATOR_INFO {
            ...
            public String toString() {
                return "Scharr Operator\n\nThis operator allows you to"
                        + " detect edges of an image in both horizontal and vertaical direction."
                        + " Moreover it is a second order derivative."
                        + " In order to apply this operation you need to use Scharr() function in OpenCV.";
            }
        }
    }
